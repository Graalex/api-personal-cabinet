...
const beneficiariesInfo = (pool, ls) => {
	return new Promise((resolve, reject) => {
		pool.get((err, db) => {
			...
			const query = `
				select pt.name as lgname, pt.percentage as lgprc, p.privpeopcnt as lgkol, p.privtaxid as lginn,
...
const allocByMonth = (pool, ls) => {
	return new Promise((resolve, reject) => {
		...
		pool.get((err, db) => {
			...
			const query = `
				select sv.god as year_s, sv.mes as month_s, sum(coalesce(ss.sumic,0)) as sumic,
					sum(coalesce(ss.retsum,0)) as retsum, sum(coalesce(v,0)) as v,
					sum(coalesce(ss.energykvt,0)) as energykvt, sum(coalesce(ss.energygcal,0)) as energygcal,
					sum(coalesce(ss.energymj,0)) as energymj, sum(coalesce(ss.standardv,0)) as standardv
				from svod_god_mes sv
				left outer join (
					select a.peracc, c.calcdate, extract (month from c.calcdate) as month_s,
						...
						c.energykvt, c.energygcal, c.energymj, c.standardv
					from calc c
					join abon a on a.kod = c.kodr
					where a.peracc = ${fb.escape(ls)} and c.gasvendorcompanyr = 2
					order by year_s, month_s, c.calcdate
				) ss on sv.god = ss.year_s and sv.mes = ss.month_s
				group by sv.god,sv.mes
			`;
			...
			db.query(query, (err, result) => {
				...
				const allocations = result.map(item => ({
					...
					total: item.SUMIC,
					volume: item.V,
					volumeKWTH: item.ENERGYKVT,
					volumeMJ: item.ENERGYMJ,
					volumeGCAL: item.ENERGYGCAL,
					crrectVolume: item.STANDARDV,
				}));
...
const subsidiesByMonth = (pool, ls) => {
	return new Promise((resolve, reject) => {
		pool.get((err, db) => {
			...
			const query = `
				select ss.year_s,ss.month_s, sum(ss.sumic) as sumic, sum(ss.needsum) as needsum
				from (
						select a.peracc, s.datic, extract (month from s.datic) as month_s,
							extract(year from s.datic) as year_s, s.sumic, s.needsum
					from subsidy s
